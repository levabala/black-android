...
The interface supports English and Russian and follows the device language. Android's per-app language setting can also select Russian for Black. The App exceptions screen includes a five-step screenshot tutorial with images matching the app language; a [Russian illustrated guide](docs/exceptions-ru.md) is available in this repository.
...
- **App exceptions** opens a scrollable exception list. Apps can be removed there or added from a fuzzy-searchable picker. Grant Usage Access on that screen to activate automatic pausing. If Android shows **Controlled by restricted setting** for this downloaded APK, open **Black app info**, use the three-dot menu to choose **Allow restricted settings**, then return and grant Usage Access. Android requires this confirmation in system settings; Black cannot grant it itself.
- Black checks public GitHub Releases in the background about once a day when the phone has a network connection. A newer APK triggers one **App updates** notification per version, with an **Update** action. Tapping it opens Black to download and verify the APK, then requests an Android self-update. Eligible later updates can install without the usual confirmation; Android still asks for confirmation when required. The first update may require allowing installs from Black in Android settings. **Check for updates** remains available for an immediate manual check. The update check uses Android's job scheduler, so the exact check time may vary with power and network conditions.
...
Black uses separate Android notification channels: **Black schedule** for the required running-service card, **Blackout warnings** for the final Cancel alert, and **App updates** for new releases. In the phone's **Settings → Apps → Black → Notifications**, some devices let you silence or turn off only **Black schedule** while leaving the other alerts enabled. Android can still list the foreground service under **Active apps**. Disabling all Black notifications hides the alerts.
...
The in-app updater expects a public release tagged like `v1.0.5` with an asset named `black-android-v1.0.5-debug.apk` or `black-android-v1.0.5.apk`. Increment both `versionCode` and `versionName` for each APK. Android requires the new APK to use the same signing key as the installed one; the updater checks this before staging the install. These preview releases use a debug signing key, so builds from a different machine or a regenerated debug key cannot update an existing installation. Android can still show its installation confirmation, and Play Protect may ask to scan an unfamiliar APK.
